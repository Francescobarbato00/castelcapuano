      ...
      const ChatBot = () => {
        ...
        const [chatSize, setChatSize] = useState("w-96 h-[600px]");
        ...
        useEffect(() => {
          const checkScreenSize = () => {
            ...
            if (window.innerWidth < 1024) {
              setChatSize("w-80 h-[500px]");
            } else if (window.innerWidth < 1280) {
              setChatSize("w-96 h-[550px]");
            } else {
              setChatSize("w-[400px] h-[600px]");
            }
          };
          checkScreenSize();
        ...
        return (
          <>
            ...
            {isOpen && (
              <div
                className={`fixed ${isMobile ? "inset-0 w-full h-full" : `bottom-24 right-8 ${chatSize} rounded-lg`} bg-white shadow-2xl flex flex-col z-[101] transition-all duration-500 ${isMobile ? "animate-fadeInMobile" : "animate-fadeInDesktop"}`}
              >
                ...
                >
                  {messages.map((msg) => (
                    ...
                    >
                      <div
                        className={`p-3 rounded-xl max-w-xs shadow-md ${msg.sender === "user" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-800"}`}
                      >
                        {msg.text}
                      </div>
                    </div>
                ...
                <div className="p-3 border-t border-gray-300 bg-white">
                  <div className="flex flex-wrap gap-2 mb-3">
                    <button onClick={() => handlePredefinedQuestion("Chi siamo?")} className="bg-blue-500 text-white px-4 py-2 rounded-full text-sm hover:bg-blue-600 transition">Chi siamo?</button>
                    <button onClick={() => handlePredefinedQuestion("Dove siamo?")} className="bg-blue-500 text-white px-4 py-2 rounded-full text-sm hover:bg-blue-600 transition">Dove siamo?</button>
                    <button onClick={() => handlePredefinedQuestion("Come contattarci?")} className="bg-blue-500 text-white px-4 py-2 rounded-full text-sm hover:bg-blue-600 transition">Come contattarci?</button>
                  </div>
                </div>
              </div>
            )}
